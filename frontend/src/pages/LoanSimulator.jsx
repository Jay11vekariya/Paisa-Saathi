...
const burdenExplanation=result=>{
 const current=result?.current_financial_snapshot?.emi_burden_pct,projected=result?.projected_financial_snapshot?.emi_burden_pct;
 if(current==null||projected==null)return '';
 const change=projected-current,label={'SAFE':'safe','CAUTION':'caution','HIGH IMPACT':'high impact','CRITICAL':'critical'}[result.impact_level]||'this result';
 if(change>0)return `Why ${label}? EMI burden rises by ${change.toFixed(1)} percentage points, reducing your monthly flexibility.`;
 if(change<0)return `Why ${label}? EMI burden decreases by ${Math.abs(change).toFixed(1)} percentage points, improving your monthly flexibility.`;
 return `Why ${label}? EMI burden remains unchanged at ${projected.toFixed(1)}% of monthly income.`;
};
