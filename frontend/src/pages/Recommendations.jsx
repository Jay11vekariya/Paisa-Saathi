  ...
  return <><div className="page-heading dashboard-heading"><div><span className="eyebrow">EXPLAINABLE NEXT STEPS</span><h1>Recommendations for your situation.</h1><p>Guidance based on your calculated cash flow — never a sales target.</p></div><CustomerSwitcher/></div>
    {loading || status !== 'connected' ? <DataState status={loading ? 'loading' : 'error'} retry={() => setRetryCount(count => count + 1)}/> : <>
      ...
      <div className="recommendation-signals">
        <div><small>Recent financial state</small><strong>{data.financial_state}</strong></div>
        <div><small>Estimated stress</small><strong>{data.financial_stress?.status === 'available' ? `${data.financial_stress.stress_level} · ${data.financial_stress.stress_score}/100` : 'Insufficient data'}</strong></div>
        <div><small>Longer-term profile</small><strong>{data.segmentation?.segment || 'Not enough data'}</strong></div>
      </div>
