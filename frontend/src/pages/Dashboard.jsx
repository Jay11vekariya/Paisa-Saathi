...
export default function Dashboard() {
  ...
  useEffect(()=>{if(status!=='connected')return;let active=true;const controller=new AbortController();getAlerts({signal:controller.signal}).then(data=>{if(active)setProactive(data.alerts.find(item=>item.status!=='DISMISSED')||null)}).catch(()=>{if(active)setProactive(null)});return()=>{active=false;controller.abort()}},[status,customer?.customer_id]);
  ...
  return <>
    ...
    {status!=='connected'?<DataState status={status} retry={retry}/>:<>
      {proactive&&<Link to="/alerts" className={`glass proactive-card severity-${proactive.severity.toLowerCase()}`}><span className="round-icon">{proactive.type==='POSITIVE_FINANCIAL_MOMENT'?<Sparkles/>:<ShieldCheck/>}</span><div><span className="eyebrow">SAATHI NOTICED SOMETHING</span><strong>{proactive.title}</strong><p>{proactive.message}</p></div><span>View Alerts <MoveUpRight size={16}/></span></Link>}
