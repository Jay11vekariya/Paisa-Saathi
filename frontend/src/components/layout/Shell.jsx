...
export default function Shell() {
  ...
  const { customer, status, dashboard } = useDashboard();
  ...
  return <div className="app-shell">
    ...
    <aside ref={menuRef} className={`sidebar ${open?'is-open':''}`} aria-label="Main navigation">
      ...
      <div className="sidebar-bottom">
        ...
        <div className="sidebar-protection"><ShieldCheck size={14}/><span>Synthetic profiles only</span></div>
    ...
    <div className="workspace">
      ...
      {panel && <div className="popover" role="region" aria-label="Account information"><button className="icon-button" aria-label="Close panel" onClick={()=>setPanel('')}><X size={16}/></button><strong>{panel==='notifications'?'A quieter kind of banking':panel==='language'?'Made for Bharat':'Your synthetic profile'}</strong><p>{panel==='notifications'?'There are no live notifications. Security monitoring is planned for a later phase.':panel==='language'?'English is available now. Gujarati and Hindi conversational support are planned for a later phase.':`${customer?.name || 'This profile'} uses fictional data. ${status==='connected'?(dashboard?.source==='test-fixture'?'This is an isolated UI test fixture.':'Financial analysis is connected to MongoDB.'):'Financial data is currently unavailable.'}`}</p>{panel==='settings' && <NavLink to="/login" onClick={()=>setPanel('')}><LogOut size={16}/> Return to login</NavLink>}</div>}
